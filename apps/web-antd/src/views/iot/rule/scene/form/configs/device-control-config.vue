...
const serviceJsonConfig = computed(() => {
...
function handleServiceSelectChange(value: any) {
  action.value.identifier = value;
...
async function fetchThingModelTSL(productId: number) {
  ...
  try {
    const res = await getThingModelTSL(productId);
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <!-- 服务选择 - 服务调用类型时显示 -->
    <div v-if="action.productId && isServiceInvokeAction" class="space-y-4">
      <Form.Item label="服务" required>
        <Select
          ...
          @change="handleServiceSelectChange"
        >
          <Select.Option
      ...
      <!-- 服务参数配置 -->
      <div v-if="action.identifier" class="space-y-4">
        <Form.Item label="服务参数" required>
          <JsonParamsInput
            v-model="paramsValue"
            type="service"
            :config="serviceJsonConfig"
